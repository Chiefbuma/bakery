...
export default function AdminOffersPage() {
  ...
  const handleUpdate = async () => {
    ...
    toast({ title: "Offer Published", description: "The daily special has been updated on the landing page." });
    fetchData();
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground font-medium">Control the premium offer featured on your storefront.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <Card className="border-none shadow-sm">
          ...
          <CardContent className="p-8 space-y-6">
             <div className="space-y-3">
                <Label className="font-black text-xs uppercase tracking-widest">Select Featured Cake</Label>
             ...
             <div className="space-y-3">
                <Label className="font-black text-xs uppercase tracking-widest">Discount Percentage (%)</Label>
                <Input 
                  type="number" 
                  value={discountPercent} 
                  onChange={(e) => setDiscountPercent(parseInt(e.target.value))} 
                  className="h-12 border-2 text-lg font-black"
                />
             </div>

             <Button className="w-full h-14 text-lg font-black gap-3 shadow-xl mt-4" onClick={handleUpdate}>
        ...
        <div className="space-y-4">
           ...
           <Card className="border-none shadow-2xl bg-stone-900 text-white overflow-hidden relative">
              <div className="p-8 space-y-6 relative z-10">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Active Offer</p>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                       <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-1">Old Price</p>
                       <p className="text-lg font-black text-stone-500 line-through">{formatPrice(currentOffer.original_price)}</p>
                    </div>
                    <div className="p-4 bg-primary/10 rounded-2xl border border-primary/20">
                       <p className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">Special Price</p>
                       <p className="text-2xl font-black text-primary">{formatPrice(currentOffer.special_price)}</p>
                    </div>
                 </div>

                 <div className="p-4 bg-white/5 rounded-2xl border border-white/10 text-center">
                    <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">Customer Savings</p>
