...
import { Star, ShoppingBasket, ArrowRight, Sparkles, Search, Loader2, Utensils, ChevronDown } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <header className="fixed top-0 w-full z-50 bg-white/70 backdrop-blur-xl border-b border-stone-200">
        <div className="container mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-black font-headline tracking-tighter hidden sm:block">WhiskeDelights</span>
          </div>
          <nav className="hidden md:flex items-center gap-8 text-[11px] font-black uppercase tracking-widest text-stone-500">
          ...
          <div className="flex items-center gap-4">
            <Link href="#menu">
              <Button className="rounded-full font-black px-6 h-10 text-[11px] shadow-xl shadow-primary/20">Order Now</Button>
            </Link>
          </div>
        </div>
      </header>
      ...
      <section className="relative min-h-screen flex items-center pt-16 overflow-hidden bg-stone-950">
        ...
        <div className="container mx-auto px-6 relative z-10 grid lg:grid-cols-2 gap-12 lg:gap-24 items-center py-12 md:py-20">
          <motion.div 
          ...
          >
            <div className="bg-white/5 backdrop-blur-3xl border border-white/10 p-8 md:p-12 rounded-[2.5rem] shadow-2xl space-y-6">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/20 border border-primary/30 text-primary font-black text-[9px] uppercase tracking-[0.2em]">
                <Sparkles className="h-3 w-3 animate-pulse" />
                <span>Nyeri's Finest Bakery</span>
              </div>
              
              <div className="space-y-4">
                <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white leading-[1] font-headline tracking-tighter">
                  Baking <br/>
                  <span className="text-primary italic">Dreams Daily</span>
                </h1>
                <p className="text-base md:text-lg text-stone-400 max-w-lg leading-relaxed font-medium">
                  We handcraft every creation using time-honored techniques to transform your moments into masterpieces.
                </p>
              </div>

              <div className="flex flex-col sm:row gap-3 pt-2">
                <Link href="#menu" className="flex-1">
                  <Button className="w-full h-14 text-base font-black rounded-xl shadow-2xl shadow-primary/20">Explore Gallery</Button>
                </Link>
                <Link href="#menu" className="flex-1">
                  <Button variant="outline" className="w-full h-14 text-base font-black rounded-xl border-white/20 text-white hover:bg-white/10">Custom Order</Button>
                </Link>
              </div>
            </div>
          </motion.div>
              ...
              <div className="relative group p-4 md:p-6 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[3rem] shadow-2xl">
                <div className="relative aspect-square w-full rounded-[2rem] overflow-hidden border-4 border-white/5 shadow-2xl bg-stone-900 mb-6">
                  ...
                  <div className="absolute top-4 right-4 h-20 w-20 md:h-24 md:w-24 bg-primary rounded-full flex flex-col items-center justify-center text-white shadow-2xl rotate-12 z-20 border-2 border-white/20 animate-bounce">
                    ...
                    <span className="text-2xl md:text-3xl font-black">{specialOffer.discount_percentage}%</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 px-2">
                  <div className="space-y-1">
                    ...
                    <h3 className="text-2xl md:text-3xl font-black text-white font-headline">{specialOffer.cake.name}</h3>
                    <div className="flex items-baseline gap-2">
                      <span className="text-primary font-black text-2xl">{formatPrice(specialOffer.special_price)}</span>
                      <span className="text-stone-500 text-sm line-through font-bold">{formatPrice(specialOffer.original_price)}</span>
                    </div>
                  </div>
                  <Link href={`/cakes/${specialOffer.cake.id}`} className="w-full md:w-auto">
                    <Button className="w-full h-14 px-8 text-base font-black rounded-xl bg-white text-stone-950 hover:bg-primary hover:text-white transition-all group">
                      Claim
      ...
      <section id="menu" className="py-20 md:py-24 container mx-auto px-6">
        <div className="flex flex-col gap-8 mb-16">
          ...
          >
            <h2 className="text-4xl md:text-6xl font-black font-headline tracking-tighter">Artisanal Gallery</h2>
            <p className="text-muted-foreground font-medium text-sm max-w-xl">Browse our collection of signature recipes designed to make your celebrations memorable.</p>
          </motion.div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="relative w-full md:w-80">
              ...
              <Input 
                ...
                className="pl-12 h-12 rounded-xl border-2 focus:border-primary/50 text-sm"
                value={searchQuery}
            ...
            <div className="flex flex-wrap gap-2 overflow-x-auto pb-2 scrollbar-hide">
              {categories.map(cat => (
                <Button 
                  ...
                  className="rounded-full px-6 font-black h-10 text-[10px] shadow-sm whitespace-nowrap"
                >
                  {cat}
                </Button>
              ))}
                  ...
                  <Card className="group h-full flex flex-col overflow-hidden border-none bg-white shadow-sm hover:shadow-xl transition-all duration-500 rounded-[2rem]">
                    <div className="relative aspect-[4/3] overflow-hidden">
                      ...
                      <div className="absolute top-4 right-4">
                        <Badge className="bg-white/95 text-black border-none px-3 py-1 font-black shadow-lg rounded-full text-[9px]">
                    ...
                    <CardContent className="p-6 flex-1 flex flex-col justify-between">
                      <div className="mb-6">
                        <span className="text-[9px] font-black text-primary uppercase tracking-widest block mb-1">{cake.category}</span>
                        <h3 className="text-xl md:text-2xl font-black mb-2 font-headline">{cake.name}</h3>
                        <p className="text-muted-foreground text-xs line-clamp-2">{cake.description}</p>
                      </div>
                      <div className="flex items-center justify-between pt-4 border-t border-stone-100">
                        <span className="text-xl font-black text-primary">{formatPrice(cake.base_price)}</span>
                        <Link href={`/cakes/${cake.id}`} onClick={() => setIsNavigating(true)}>
                          <Button size="sm" className="rounded-xl gap-2 font-black px-5 h-10 text-[11px]">
                            {isNavigating ? <Loader2 className="h-3 w-3 animate-spin" /> : (cake.customizable ? 'Customize' : 'Order')}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            <div className="lg:col-span-2 space-y-6">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 bg-primary rounded-2xl flex items-center justify-center shadow-xl shadow-primary/20">
                  <Utensils className="h-5 w-5 text-white" />
                </div>
                <span className="text-2xl font-black font-headline tracking-tighter">WhiskeDelights</span>
              </div>
              <p className="text-muted-foreground font-medium text-xs max-w-sm leading-relaxed">Crafting premium artisanal cakes for Nyeri County and beyond. Every slice tells a story of passion and quality.</p>
              <div className="flex gap-3">
                <Link href="#" className="h-10 w-10 rounded-xl bg-white border shadow-sm flex items-center justify-center hover:bg-primary hover:text-white transition-all group">
                  <InstagramIcon className="h-5 w-5 group-hover:scale-110 transition-transform" />
                </Link>
                <Link href="#" className="h-10 w-10 rounded-xl bg-white border shadow-sm flex items-center justify-center hover:bg-green-600 hover:text-white transition-all group">
            ...
            <div className="space-y-4">
              <h4 className="font-black uppercase tracking-widest text-[10px] text-stone-400">Navigation</h4>
            ...
            <div className="space-y-4">
              <h4 className="font-black uppercase tracking-widest text-[10px] text-stone-400">Visit Us</h4>
              <ul className="space-y-3 text-xs font-bold text-muted-foreground">
